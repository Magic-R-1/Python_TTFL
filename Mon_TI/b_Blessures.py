    ...
    if response.status_code == 200:
        ...
        for i in range(0, len(all_players), 4):
            ...
            try:
                ...
                # Gestion de player_status pour lui affecter Probable, Questiobale, ou Doubtful si statut original en Day-To-Day
                player_status = gestion_status_probable_questiobale_doubtful(player_status, player_comment)

                liste_data_blessure_joueurs.append([player_name, player_id, player_returnDate, player_status, player_comment])
            
            except Exception as e:
                print(f"Erreur lors du traitement du joueur {player_name}, joueur absent des données de l'API NBA : {e}")
...
def gestion_status_probable_questiobale_doubtful(player_status, player_comment):
    ...
    if player_status == 'Day-To-Day':
        # Vérification si player_comment contient les mots clés et affectation de new_player_status en conséquence
        if "probable" in player_comment.lower():    # mettre en minuscule pour ne pas tenir conmpte de la casse dans le test
            player_status = "Probable"
        elif "questionable" in player_comment.lower():
            player_status = "Questionable"
        elif "doubtful" in player_comment.lower():
...
# ------------------------------
# Fonction pour fusionner la liste de joueur avec les données ESPN
def obtenir_DF_joueurs_status(ids_joueurs):
    ...
    DF_joueurs_status['Joueur'] = DF_joueurs_status['ID'].apply(obtenir_joueurNom_avec_joueurID)
